...
//         '<img id="loading" src="http://bit.ly/pMtW1K">' +
    ...
    function refreshDAonHDMS(){
    	var titolo = $('#title').text()
    	var from
    	var to
//     	console.log(titolo)
    	var tot = titolo.length
    	
    	if(titolo.includes(":")){
//     		console.log("Custom")
    		var primiDuePunti = titolo.indexOf(":")
    		from = titolo.slice(primiDuePunti, primiDuePunti+10);
    	}
    	else{
//     		console.log("Predefined")
    		from=titolo.slice(0,10)
    	}
    	
    	to=titolo.slice(tot-10,tot)
    	$.ajax({
			type : "get",
			url : "refreshDailyActivities/" + from + "/"
					+ to,
			success : function(msg) {
				console.log(msg)
			}
    ...
    function checkExProgrammeObsolescence(){
    	var exTimeInMillis = ${exProgramme.date.getTime()}
    	var now=moment();
    	var exDate = moment(exTimeInMillis)
    	var diff = now.diff(exDate, 'days')
    	console.log(diff)
    	if (diff >7){
    		$("#obsolete").show()		    		
    	}else{

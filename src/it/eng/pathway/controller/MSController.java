...
@Controller
@SessionAttributes({ "xSessionId", "sin", "username", "mstoken", "mscode", "msrefresh","patient" ,"lastPresent", "mssessions", "activitysessions", "notTagged", "aLabels", "msError"})
@Transactional
...
////	private static final String TOKEN_URL = "https://login.live.com/oauth20_token.srf";
	...
	public String mslogin(Model model, Locale locale) {
		try {
			...
			model.asMap().clear(); //senza questo passava tutto il model alla pagina di MICROSOFT per la login
...
//		altrimenti reindirizzarlo alla pagina per l'oauth
		...
		try {
			...
			if (model.asMap().get("msError") != null){
				model.addAttribute("msError", null);
			}
		} catch (UnauthorizedException e1) {
			...
			CloudLogger.logger.fine(e1.toString());
			toRefresh=true;
			if (e1.getCode() == 500){
				PwLogger.logger.fine(e1.getCode()+" MSAPI is DOWN - Internal Server Error"+" "+e1.getMsg());
				toRefresh = false;
				model.addAttribute("msError", e1.getCode());
			}
		}
		if (toRefresh){
			try {
				...
				PwLogger.errorLogger.severe(e.toString());
				PwLogger.logger.severe("[ERROR] - IOException - during REFRESHING the TOKEN Check the errors log");
				e.printStackTrace();
				PwLogger.logger.severe("DELETING THE TOKENS FOR THE PATIENT: "+patient.getSin());
				patient.setMsRefreshToken(null);
				patient.setMsToken(null);
				throw new NoRefreshTokenException();
			} catch (UnauthorizedException e) {
				PwLogger.errorLogger.severe(e.toString());
				PwLogger.errorLogger.severe(e.getCode()+" "+e.getMsg());
				PwLogger.logger.severe("[ERROR] - UnauthorizedException - during REFRESHING the TOKEN Check the errors log. Maybe is needed the signin again");
				e.printStackTrace();
				PwLogger.logger.severe("DELETING THE TOKENS FOR THE PATIENT: "+patient.getSin());
				patient.setMsRefreshToken(null);
				patient.setMsToken(null);
...
//		una volta inizializzata la connessione
		...
		try {
			...
			PwLogger.logger.fine("[ERROR] Check the errors log");
			if (e.getCode() == 500){
				PwLogger.logger.fine(e.getCode()+" MSAPI is DOWN - Internal Server Error"+" "+e.getMsg());
				model.addAttribute("msError", e.getCode());
			}
//			e.printStackTrace();
		}
...
//		una volta inizializzata la connessione
		...
		String token=(String) model.asMap().get("mstoken");
		try {
//			PwLogger.logger.fine("[DEBUG] Trying FROM:"+from+" TO:"+to);
			if (from.before(to)){
				summary=getGateway().getSummary(token, from, to);
			}
		} catch (UnauthorizedException e) {
...
//			e.printStackTrace();
	...
	public List<DailyActivity> getDailyActivities(Date from, Date to,  Model model, Locale locale){
...
//		una volta inizializzata la connessione
		...
		String token=(String) model.asMap().get("mstoken");
		try {
			das=getGateway().getDailyActivities(token, from, to);
		} catch (UnauthorizedException e) {
			PwLogger.errorLogger.fine(e.toString());
			PwLogger.logger.fine("[ERROR] Check the errors log");
//			e.printStackTrace();
